# -*- coding: utf-8 -*-
"""
CatBoost tabulaire + visuel PCA 256 pour la prédiction du prix Airbnb.

Objectif :
- utiliser le fichier tabulaire nettoyé avec sentiments 
- utiliser les embeddings visuels EfficientNet-B0 
- respecter le split déjà défini dans split_listing_ids.csv 
- appliquer une PCA 256 sur les embeddings visuels 
- entraîner CatBoost sur tabulaire + composantes visuelles 
- évaluer sur log_price puis en euros 
- sauvegarder les mêmes types de rapports que le modèle tabulaire seul.
...
class Config:
    ...
    embeddings_npy_path: str = "Resultat2/efficientnet_b0_embeddings.npy"
    embeddings_ids_path: str = "Resultat2/efficientnet_b0_embeddings_ids.csv"

    # Dossier principal des résultats
    output_dir_name: str = "Rapport_Catboost_Tabulaire_Visuelle"
    ...
    cols_to_exclude: Tuple[str, ...] = (
        # Identifiants / URLs / images / index
        "id",
        "id_clean",
        "listing_id_clean",
        "listing_url",
        ...
        # Colonnes retirées dans les expériences précédentes
        "has_reviews",
        "nb_avis_textuels_bert",
        "bert_stars_moyen",

        # Prédictions texte éventuelles
        "text_pred_final",
        "text_pred_oof",
        "text_pred_test",
    )

    # On retire les embeddings texte et les embeddings image bruts s'ils existent déjà dans un CSV.
    # Ici on ajoute les composantes PCA proprement depuis le NPY.
    ...
    # Colonnes textuelles brutes à exclure
    ...
    task_type: str = "CPU"
    thread_count: int = -1
    used_ram_limit: Optional[str] = "18gb"
...
def get_paths(cfg: Config) -> Dict[str, Path]:
    ...
    paths = {
        ...
        "embeddings_npy_path": script_dir / cfg.embeddings_npy_path,
        "embeddings_ids_path": script_dir / cfg.embeddings_ids_path,
...
# 4. CHARGEMENT ET ALIGNEMENT TABULAIRE + SPLIT + EMBEDDINGS
...
def load_and_align_data(cfg: Config, paths: Dict[str, Path]) -> Dict[str, Any]:
    ...
    for name in ["tabular_path", "split_path", "embeddings_npy_path", "embeddings_ids_path"]:
        if not paths[name].exists():
            raise FileNotFoundError(f"Fichier introuvable : {paths[name]}")

    df = pd.read_csv(
        paths["tabular_path"],
        dtype={cfg.id_col: str},
        low_memory=False,
    )

    split_df = pd.read_csv(paths["split_path"], dtype=str)
    emb_ids = pd.read_csv(paths["embeddings_ids_path"], dtype=str)
    emb_npy = np.load(paths["embeddings_npy_path"], mmap_mode="r")

    print("Tabulaire :", df.shape)
    print("Split :", split_df.shape)
    print("Embeddings IDs :", emb_ids.shape)
    print("Embeddings NPY :", emb_npy.shape)
    ...
    required_emb_cols = {"row_npy", "listing_id_clean"}
    missing_cols = required_emb_cols - set(emb_ids.columns)
    if missing_cols:
        raise ValueError(f"Colonnes manquantes dans embeddings_ids : {missing_cols}")

    if len(emb_ids) != emb_npy.shape[0]:
        raise ValueError(
            f"Problème d'alignement : embeddings_ids contient {len(emb_ids)} lignes, "
            f"mais le NPY contient {emb_npy.shape[0]} lignes."
        )

    if emb_npy.shape[1] != 1280:
        raise ValueError(f"Dimension EfficientNet-B0 attendue : 1280, trouvé : {emb_npy.shape[1]}")

    # Nettoyage IDs
    df[cfg.id_col] = normalize_id_series(df[cfg.id_col])
    split_df[split_id_col] = normalize_id_series(split_df[split_id_col])
    emb_ids["listing_id_clean"] = normalize_id_series(emb_ids["listing_id_clean"])
    emb_ids["row_npy"] = pd.to_numeric(emb_ids["row_npy"], errors="raise").astype(int)

    # Source du split = split_listing_ids.csv
    ...
    # On retire les colonnes split déjà présentes pour éviter les doublons
    ...
    # Merge embeddings IDs
    emb_ids_clean = emb_ids[["listing_id_clean", "row_npy"]].copy()
    if "image_path" in emb_ids.columns:
        emb_ids_clean["image_path"] = emb_ids["image_path"]

    emb_ids_clean = emb_ids_clean.rename(columns={"listing_id_clean": cfg.id_col})
    ...
    if row_indices.min() < 0 or row_indices.max() >= emb_npy.shape[0]:
        raise ValueError("row_npy contient des index hors limites par rapport au NPY.")

    X_img = np.asarray(emb_npy[row_indices], dtype=np.float32)

    if len(df) != X_img.shape[0]:
        raise ValueError("Le nombre de lignes tabulaires alignées ne correspond pas au nombre d'embeddings.")

    print("\n================ ALIGNEMENT FINAL ================")
    print("Lignes après alignement tabulaire + split + visuel :", df.shape)
    print("Matrice visuelle alignée :", X_img.shape)
    print("Répartition split :")
    print(split_counts)

    # Sauvegarde audit alignement
    audit_alignment = {
        "tabular_rows_after_alignment": int(len(df)),
        "visual_matrix_shape": list(X_img.shape),
        "n_train_dev": int((df["split"] == "train_dev").sum()),
        "n_test_final": int((df["split"] == "test_final").sum()),
        "embedding_dim_original": int(X_img.shape[1]),
        "pca_components": int(cfg.pca_components),
    }

    with open(paths["reports_dir"] / "00_audit_alignement_tabulaire_visuel.json", "w", encoding="utf-8") as f:
        json.dump(audit_alignment, f, indent=2, ensure_ascii=False)

    df[[cfg.id_col, "split", "row_npy"]].to_csv(
        paths["reports_dir"] / "00_ids_alignement_tabulaire_visuel.csv",
        index=False,
        encoding="utf-8-sig",
    )
...
# 5. AUDIT ET PREPARATION FEATURES TABULAIRES
...
def prepare_tabular_features(df: pd.DataFrame, cfg: Config, paths: Dict[str, Path]) -> Dict[str, Any]:
    ...
    forbidden_still_present = [c for c in X_tab.columns if should_exclude_column(c, cfg)]
...
# 6. SPLIT A PARTIR DU FICHIER EXISTANT
...
def make_split_from_existing_file(data: Dict[str, Any], X_img: np.ndarray, cfg: Config, paths: Dict[str, Path]) -> Dict[str, Any]:
    ...
    if len(X_tab) != len(X_img):
        raise ValueError("X_tab et X_img n'ont pas le même nombre de lignes.")
    ...
    split_ids.to_csv(
        paths["reports_dir"] / "08_split_final_ids_utilise.csv",
        index=False,
        encoding="utf-8-sig",
    )
...
def make_pca_columns(n_components: int) -> list:
    return [f"img_pca_{i}" for i in range(n_components)]


def fit_transform_visual_pca(X_train_img, X_other_img, cfg: Config, seed: int):
    n_components = min(cfg.pca_components, X_train_img.shape[1], X_train_img.shape[0] - 1)
...
def build_fusion_dataframe(X_tab: pd.DataFrame, X_img_pca: np.ndarray, pca_columns: list) -> pd.DataFrame:
    X_img_df = pd.DataFrame(X_img_pca, columns=pca_columns)
    X_fusion = pd.concat(
        [
...
def run_kfold_cv(split_data: Dict[str, Any], cfg: Config, paths: Dict[str, Path]):
    ...
    for fold_id, (train_idx, val_idx) in enumerate(skf.split(X_tab, strat_bins), start=1):
        ...
        # PCA apprise uniquement sur le train du fold
        pca, X_img_train_pca, X_img_val_pca = fit_transform_visual_pca(
            X_img_train,
            X_img_val,
            cfg,
            seed=fold_seed,
        ...
        pca_columns = make_pca_columns(X_img_train_pca.shape[1])

        X_train = build_fusion_dataframe(X_tab_train, X_img_train_pca, pca_columns)
        X_val = build_fusion_dataframe(X_tab_val, X_img_val_pca, pca_columns)
        ...
        model_path = paths["models_dir"] / f"catboost_tabulaire_visuel_pca256_cv_{fold_name}.cbm"
        ...
        print(
            ...
            f"PCA explained = {np.sum(pca.explained_variance_ratio_):.4f}"
        )
    ...
    fold_pca_df.to_csv(
        paths["reports_dir"] / "14b_cv_pca_explained_variance.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    cv_summary = {
        "experiment": "tabular_visual_pca256_kfold",
        "n_splits": cfg.n_splits,
        ...
        "pca_fitted_inside_each_fold": True,
    ...
    with open(paths["reports_dir"] / "18_cv_resume.txt", "w", encoding="utf-8") as f:
        f.write("Résumé CV - CatBoost tabulaire + visuel PCA 256\n")
        f.write("=" * 90 + "\n\n")
        ...
        f.write(f"Nombre de composantes PCA : {cfg.pca_components}\n\n")
        ...
        f.write("\nVariance expliquée PCA par fold\n")
        f.write("-" * 90 + "\n")
        f.write(fold_pca_df.to_string(index=False))
        f.write("\n\n")

        f.write("Nombre d'arbres par fold\n")
        f.write("-" * 90 + "\n")
...
def train_final_model_and_evaluate(
    ...
    print("\n" + "=" * 100)
    print("ENTRAINEMENT FINAL - TABULAIRE + VISUEL PCA")
    print("=" * 100)
    ...
    # PCA finale apprise uniquement sur train_dev
    final_pca, X_img_train_pca, X_img_test_pca = fit_transform_visual_pca(
        X_img_train,
        X_img_test,
        cfg,
        seed=cfg.random_state + 999,
    )

    pca_columns = make_pca_columns(X_img_train_pca.shape[1])

    X_train = build_fusion_dataframe(X_tab_train, X_img_train_pca, pca_columns)
    X_test = build_fusion_dataframe(X_tab_test, X_img_test_pca, pca_columns)
    ...
    pca_report.to_csv(
        paths["pca_dir"] / "final_pca_explained_variance.csv",
        index=False,
        encoding="utf-8-sig",
    )

    pd.DataFrame({"feature": feature_names}).to_csv(
        paths["reports_dir"] / "19_features_finales_tabulaire_visuel_pca.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    final_model_path = paths["models_dir"] / "catboost_final_tabulaire_visuel_pca256_manual_aggressive_c05.cbm"
    final_model.save_model(str(final_model_path))

    print("Modèle final sauvegardé :", final_model_path)
    print("PCA finale sauvegardée :", pca_path)
    ...
    model_config = {
        ...
        "important": (
            "Modèle tabulaire + visuel. "
            "Les embeddings EfficientNet-B0 sont réduits par PCA à 256 composantes. "
            "La PCA est apprise uniquement sur train_dev pour le modèle final, "
            "et uniquement sur le train de chaque fold pendant la CV. "
...
# ============================================================
# 13. IMPORTANCE, SHAP, GRAPHIQUES, RAPPORTS
# ============================================================
...
def save_feature_importance(model, feature_names, paths: Dict[str, Path]):
    ...
    plt.title("CatBoost tabulaire + visuel PCA 256 - Top 30 variables")
    plt.tight_layout()
...
def save_feature_group_importance(model, feature_names, paths: Dict[str, Path]):
    ...
    df_imp["group"] = np.where(
        df_imp["feature"].str.startswith("img_pca_"),
        "visuel_pca",
        "tabulaire",
    )
    ...
    if total > 0:
        group_imp["importance_pct"] = group_imp["importance"] / total * 100
    else:
        group_imp["importance_pct"] = np.nan

    group_imp.to_csv(
        paths["reports_dir"] / "24b_importance_groupes_tabulaire_vs_visuel.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    plt.title("Importance globale : tabulaire vs visuel PCA")
    plt.tight_layout()
    plt.savefig(paths["plots_dir"] / "24b_importance_groupes_tabulaire_vs_visuel.png", dpi=160)
    plt.close()
...
def save_shap_final(model, X_train, y_train_log, cat_features, cfg: Config, paths: Dict[str, Path]):
    ...
    shap_df["group"] = np.where(
        shap_df["feature"].str.startswith("img_pca_"),
        "visuel_pca",
        "tabulaire",
    )

    shap_group = (
        shap_df.groupby("group", as_index=False)["mean_abs_shap"]
        .sum()
        .sort_values("mean_abs_shap", ascending=False)
    )
    ...
    if total > 0:
        shap_group["mean_abs_shap_pct"] = shap_group["mean_abs_shap"] / total * 100
    else:
        shap_group["mean_abs_shap_pct"] = np.nan

    shap_group.to_csv(
        paths["shap_dir"] / "final_shap_groupes_tabulaire_vs_visuel.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    plt.title("CatBoost tabulaire + visuel PCA 256 - Top 30 SHAP")
    plt.tight_layout()
...
def write_final_report(
    ...
    report_path = paths["reports_dir"] / "30_resume_final_tabulaire_visuel_pca256_catboost.txt"

    with open(report_path, "w", encoding="utf-8") as f:
        f.write("Résumé final - CatBoost tabulaire + visuel PCA 256\n")
        f.write("=" * 100 + "\n\n")

        f.write("Candidat retenu\n")
        f.write("-" * 100 + "\n")
        f.write("manual_aggressive__C05_premium_oriented + EfficientNet-B0 PCA 256\n\n")
        ...
        f.write("Le test final n'est jamais utilisé pendant la CV.\n")
        f.write("Les embeddings visuels viennent d'EfficientNet-B0 et ont une dimension initiale de 1280.\n")
        f.write("Une PCA réduit ces embeddings à 256 composantes.\n")
        f.write("Pendant la CV, la PCA est apprise uniquement sur le train de chaque fold.\n")
        f.write("Pour le modèle final, la PCA est apprise uniquement sur train_dev puis appliquée à test_final.\n")
        ...
        f.write(f"Composantes visuelles PCA : {cfg.pca_components}\n")
        f.write(f"Variance expliquée PCA finale : {np.sum(final_pca.explained_variance_ratio_):.6f}\n\n")
        ...
        f.write("-" * 100 + "\n")
        f.write(
            "Cette expérience mesure l'apport de la modalité visuelle seule, "
            "ajoutée au socle tabulaire et sentiment. "
            "Si les métriques test final s'améliorent par rapport au tabulaire, "
            "cela indique que l'image apporte un signal complémentaire. "
            "Si l'amélioration est faible ou absente, cela peut signifier que les informations visuelles "
            "sont déjà partiellement capturées par les variables tabulaires, ou que l'embedding générique "
            "EfficientNet-B0 n'est pas suffisamment spécialisé pour le prix Airbnb.\n"
        )
...
def main():
    ...
    print("\n" + "=" * 100)
    print("CATBOOST TABULAIRE + VISUEL PCA 256 - K-FOLD CV - MANUAL AGGRESSIVE C05")
    print("=" * 100)
    print("Dossier script :", paths["script_dir"])
    print("Fichier tabulaire :", paths["tabular_path"])
    print("Fichier split :", paths["split_path"])
    print("Fichier embeddings NPY :", paths["embeddings_npy_path"])
    print("Fichier embeddings IDs :", paths["embeddings_ids_path"])
    ...
    prepared = prepare_tabular_features(df, CFG, paths)
        ...
        json.dump(
            to_jsonable({
                ...
                "important": (
                    ...
                    "PCA apprise sans fuite de données. "
                    "K-fold uniquement sur train_dev. "
    ...
    print("\n" + "=" * 100)
    print("FIN - CATBOOST TABULAIRE + VISUEL PCA 256")
    print("=" * 100)
    print("Résultats sauvegardés dans :")
    print(paths["output_dir"])

    print("\nFichiers importants à m'envoyer pour analyse :")
    print(paths["reports_dir"] / "00_audit_alignement_tabulaire_visuel.json")
    print(paths["reports_dir"] / "03_features_tabulaires_utilisees.csv")
    print(paths["reports_dir"] / "05_colonnes_exclues.csv")
    ...
    print(paths["reports_dir"] / "14b_cv_pca_explained_variance.csv")
    ...
    print(paths["reports_dir"] / "24b_importance_groupes_tabulaire_vs_visuel.csv")
    print(paths["reports_dir"] / "30_resume_final_tabulaire_visuel_pca256_catboost.txt")
    print(paths["pca_dir"] / "final_pca_explained_variance.csv")
